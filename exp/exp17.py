...
import src.configs.config17 as config
...
from src.misspells import typo_dict
...
def correct_spelling(x, dic):
    # https://www.kaggle.com/c/tweet-sentiment-extraction/discussion/138143
    ...
    for word in dic.keys():
        if word in x_sp:
            # tokenize した時に複数の文字に分割されてしまう typo のみ修正
            # didnt, doesnt, aka など tokenizer の辞書に存在するものは修正しない
            if len(config.TOKENIZER.encode(word).ids) > 2:
...
def run_one_fold(fold_id):
    ...
    with timer('load csv data'):
        ...
        df_train['text'] = df_train['text'].progress_apply(lambda x: correct_spelling(x, typo_dict))
        df_train['selected_text'] = df_train['selected_text'].progress_apply(lambda x: correct_spelling(x, typo_dict))
