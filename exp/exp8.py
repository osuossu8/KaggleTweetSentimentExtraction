...
import src.configs.config8 as config
import src.engine as engine
from src.machine_learning_util import seed_everything, prepare_labels, DownSampler, timer, \
                                      to_pickle, unpickle
from src.model import TweetBERTBaseUncased, TweetModel, TweetModelLargeWWM
...
def run_one_fold(fold_id):
    ...
    with timer('create model'):
        ...
        model_config = transformers.BertConfig.from_pretrained("bert-large-uncased-whole-word-masking", output_hidden_states = True)
        # model_config.output_hidden_states = True

        model = TweetModelLargeWWM("bert-large-uncased-whole-word-masking", model_config)
