...
import src.engine51 as engine
from src.machine_learning_util import seed_everything, prepare_labels, timer, to_pickle, unpickle
from src.model2 import TweetRoBERTaModel, TweetRoBERTaModelSimple, TweetRoBERTaModelConv1dHead, TweetModel
...
def remove_urls(x):
    x = str(x)
    new_x = re.sub(r"(https?|ftp)(:\/\/[-_\.!~*\'()a-zA-Z0-9;\/?:\@&=\+$,%#]+)", "" ,x)
    if len(new_x.split())<2:
        return x
    return new_x

def remove_mention(x):
    x = str(x)
    new_x = ' '.join([i for i in x.split() if '@' not in i])
    if len(new_x.split())<2:
        return x
    return new_x
...
def run_one_fold(fold_id):
    ...
    with timer('load csv data'):
        ...
        # df_train.loc[df_train['sentiment']=='neutral', 'selected_text'] = df_train.loc[df_train['sentiment']=='neutral']['text']

        df_train.text = df_train.text.map(normalize)
        df_train.selected_text = df_train.selected_text.map(normalize) 
 
        #df_train.text = df_train.text.map(remove_urls)
        #df_train.text = df_train.text.map(remove_mention)
        #df_train.text = df_train.text.map(remove_hashtag)

        #df_train.selected_text = df_train.selected_text.map(remove_urls)
        #df_train.selected_text = df_train.selected_text.map(remove_mention)
        #df_train.selected_text = df_train.selected_text.map(remove_hashtag)
    ...
    with timer('create model'):
        ...
        model = TweetModel(config.ROBERTA_PATH)
