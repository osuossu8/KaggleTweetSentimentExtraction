...
import src.configs.config20 as config
...
def run_one_fold(fold_id):
    ...
    with timer('load csv data'):
        ...
        df_ex = pd.read_csv(config.EXTERNAL_PATH).dropna().reset_index(drop=True)
        df_ex = df_ex[df_ex['sentiment']!='neutral']

        len_original = len(df_train)

        df_train = pd.concat([df_train, df_ex]).reset_index(drop=True)

        if debug:
            df_train = df_train.sample(1000, random_state=SEED).dropna().reset_index(drop=True)

        ex_index = list(df_train[len_original:].index)

        num_folds = config.NUM_FOLDS
        kf = StratifiedKFold(n_splits = num_folds, random_state = SEED)
        splits = list(kf.split(X=df_train[:len_original], y=df_train[['sentiment']][:len_original]))
        train_idx = splits[fold_id][0]
        val_idx = splits[fold_id][1]

        train_idx = list(train_idx)+ex_index
    ...
    with timer('create model'):
        ...
        #pretrain_path = 'models/exp18_fold0.pth'
        #model.load_state_dict(torch.load(pretrain_path))
        #LOGGER.info(f'pretrained model (exp18) loaded')
