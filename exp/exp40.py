...
EXP_ID = "exp40"
import src.configs.config40 as config
import src.engine as engine
from src.machine_learning_util import seed_everything, prepare_labels, DownSampler, timer, \
                                      to_pickle, unpickle
from src.model import TweetBERTBaseUncased, TweetModel, TweetModelLargeWWM, TweetRoBERTaModel, TweetBERTModelMK2, TweetALBERTModel
...
def run_one_fold(fold_id):
    ...
    with timer('create model'):
        ...
        # model = TweetBERTModelMK2(config.BERT_PATH)
        model = TweetALBERTModel(config.ALBERT_PATH)
