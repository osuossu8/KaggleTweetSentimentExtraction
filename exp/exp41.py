...
EXP_ID = "exp41"
import src.configs.config41 as config
import src.engine_with_aux_noise_target as engine
from src.machine_learning_util import seed_everything, prepare_labels, DownSampler, timer, \
                                      to_pickle, unpickle
from src.model import TweetBERTBaseUncased, TweetModel, TweetModelLargeWWM, TweetRoBERTaModelV4, TweetRoBERTaModelMK4
...
def process_data_with_aux(tweet, selected_text, sentiment, tokenizer, max_len):
    ...
    # threshold = 0.6 # 0.707 
    threshold = 0.6
    noise = is_noise(selected_text, threshold)        
    if noise > threshold:
        noise_target = [1, 0]
    else:
        noise_target = [0, 1]
...
class TweetDatasetWithAux:
    ...
    def __getitem__(self, item):
        ...
        return {
            ...
            'noise_target': torch.tensor(data["noise_target"], dtype=torch.float32)
        }


def is_noise(example, threshold):
    s1 = ' '.join(example.split())
    s2 = ' '.join([config.TOKENIZER.decode([ix]).strip() for ix in config.TOKENIZER.encode(example).ids])
    
    jac = jaccard(s1, s2)
    
    return 1 if (1-jac) > threshold else 0


def get_noise(example, verbose=False):
    s1 = ' '.join(example.split())
    s2 = ' '.join([config.TOKENIZER.decode([ix]).strip() for ix in config.TOKENIZER.encode(example).ids])
...
def run_one_fold(fold_id):
    ...
    with timer('load csv data'):
        ...
        df_train['noise'] = df_train['selected_text'].apply(lambda x: get_noise(x))
        df_train = df_train[df_train.noise<0.9].reset_index(drop=True)
    ...
    with timer('create model'):
        ...
        model = TweetRoBERTaModelMK4(config.ROBERTA_PATH)
