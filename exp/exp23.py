...
import src.configs.config23 as config
import src.engine as engine
from src.machine_learning_util import seed_everything, prepare_labels, DownSampler, timer, \
                                      to_pickle, unpickle
from src.model import TweetBERTBaseUncased, TweetModel, TweetModelLargeWWM, TweetRoBERTaModel, TweetRoBERTaModelV2
...
def run_one_fold(fold_id):
    ...
    with timer('create model'):
        ...
        model = TweetRoBERTaModelV2(config.ROBERTA_PATH)
        ...
        #pretrain_path = f'models/exp21_fold{fold_id}.pth'
        #model.load_state_dict(torch.load(pretrain_path))
        #LOGGER.info(f'pretrained model (exp21) loaded')
