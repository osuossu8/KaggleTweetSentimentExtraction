...
EXP_ID = "exp30"
import src.configs.config30 as config
...
from src.machine_learning_util import seed_everything, prepare_labels, DownSampler, timer, \
                                      to_pickle, unpickle
from src.model import TweetBERTBaseUncased, TweetModel, TweetModelLargeWWM, TweetRoBERTaModelV3, TweetRoBERTaModelV4
...
def process_data_with_aux_v2(tweet, selected_text, sentiment, tokenizer, max_len):
    ...
    # sentiment 分類を aux task で解くなら Leak になるからコメントアウト
    # sentiment_id = {
    #     'positive': 1313,
    #     'negative': 2430,
    #     'neutral': 7974
...
def run_one_fold(fold_id):
    ...
    with timer('prepare validation data'):
        train_dataset = TweetDatasetWithAuxV2(
        ...
        val_dataset = TweetDatasetWithAuxV2(
    ...
    with timer('create model'):
        ...
        model = TweetRoBERTaModelV3(config.ROBERTA_PATH)
