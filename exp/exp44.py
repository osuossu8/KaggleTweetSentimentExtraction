...
EXP_ID = "exp44_incorrect_aux_target"
import src.configs.config44 as config
import src.engine_with_aux_new as engine
from src.machine_learning_util import seed_everything, prepare_labels, DownSampler, timer, \
                                      to_pickle, unpickle
from src.model import TweetBERTBaseUncased, TweetModel, TweetModelLargeWWM, TweetRoBERTaModelV3, TweetRoBERTaModelMK6
...
def process_data_with_aux(tweet, selected_text, sentiment, tokenizer, max_len, incorrect):
    ...
    if sentiment == 'positive':
        sentiment_target = [1, 0, 0]
    elif sentiment == 'negative':
        sentiment_target = [0, 1, 0]
    else:
...
class TweetDatasetWithAux:
    def __init__(self, tweet, sentiment, selected_text, incorrect):
        self.tweet = tweet
    ...
    def __getitem__(self, item):
        ...
        return {
            ...
            'incorrect_target': torch.tensor(data["incorrect_target"], dtype=torch.float32)
        }
...
def run_one_fold(fold_id):
    ...
    with timer('load csv data'):
        ...
        df_train['incorrect'] = df_train.apply(is_incorrect_fn, axis=1)
    ...
    with timer('create model'):
        ...
        model = TweetRoBERTaModelMK6(config.ROBERTA_PATH)
